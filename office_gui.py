...
说明：
- 依赖 office_converter.py 中的 OfficeConverter（你已经更新到 v5.13.0）
- GUI 中：
...
class OfficeGUI(tk.Tk):
    def __init__(self, config_path=None):
        ...
        if not os.path.exists(self.config_path):
            messagebox.showinfo(
                "提示",
                "未找到 config.json，将按默认路径生成一个。\n"
                "可以先在 GUI 中配置好后点击“保存配置”。",
            )
    ...
    def _build_ui(self):
        ...
        self.progress = ttk.Progressbar(
            status_frame, orient="horizontal", mode="indeterminate"
        )
    ...
    def _build_run_tab(self, tab_run: ttk.Frame):
        ...
        # 配置文件路径
        ttk.Label(tab_run, text="配置文件:").grid(row=row, column=0, sticky="w")
        self.var_config_path = tk.StringVar(value=self.config_path)
        ent_cfg = ttk.Entry(tab_run, textvariable=self.var_config_path, width=60)
        ent_cfg.grid(row=row, column=1, sticky="ew", padx=4)
        ttk.Button(tab_run, text="打开所在目录", command=self.open_config_folder).grid(
            row=row, column=2, padx=4
        )
        row += 1

        # 源目录
        ttk.Label(tab_run, text="源目录:").grid(row=row, column=0, sticky="w")
        self.var_source_folder = tk.StringVar()
        ent_src = ttk.Entry(tab_run, textvariable=self.var_source_folder, width=60)
        ent_src.grid(row=row, column=1, sticky="ew", padx=4)
        ttk.Button(tab_run, text="浏览...", command=self.browse_source).grid(
            row=row, column=2, padx=2
        )
        ttk.Button(tab_run, text="打开", command=self.open_source_folder).grid(
            row=row, column=3, padx=2
        )
        row += 1

        # 目标目录
        ttk.Label(tab_run, text="目标目录:").grid(row=row, column=0, sticky="w")
        self.var_target_folder = tk.StringVar()
        ent_tgt = ttk.Entry(tab_run, textvariable=self.var_target_folder, width=60)
        ent_tgt.grid(row=row, column=1, sticky="ew", padx=4)
        ttk.Button(tab_run, text="浏览...", command=self.browse_target).grid(
            row=row, column=2, padx=2
        )
        ttk.Button(tab_run, text="打开", command=self.open_target_folder).grid(
            row=row, column=3, padx=2
        )
        row += 1

        # 运行模式
        ttk.Label(tab_run, text="运行模式:").grid(row=row, column=0, sticky="w")
        self.var_run_mode = tk.StringVar(value=MODE_CONVERT_THEN_MERGE)
        frm_run = ttk.Frame(tab_run)
        frm_run.grid(row=row, column=1, columnspan=3, sticky="w")
        ttk.Radiobutton(
            frm_run,
            text="仅转换",
        ...
        self.lbl_collect_mode = ttk.Label(
            tab_run, text="梳理子模式:", foreground="#555"
        )
        self.lbl_collect_mode.grid(row=row, column=0, sticky="w")
        self.var_collect_mode = tk.StringVar(value=COLLECT_MODE_COPY_AND_INDEX)
        frm_collect = ttk.Frame(tab_run)
        frm_collect.grid(row=row, column=1, columnspan=3, sticky="w")
        ...
        # 内容策略
        self.lbl_strategy = ttk.Label(tab_run, text="内容处理策略:")
        self.lbl_strategy.grid(row=row, column=0, sticky="w")
        self.var_strategy = tk.StringVar(value="standard")
        frm_strategy = ttk.Frame(tab_run)
        frm_strategy.grid(row=row, column=1, columnspan=3, sticky="w")
        ttk.Radiobutton(
            frm_strategy,
            text="标准分类",
            value="standard",
        ...
        # 合并选项
        ttk.Label(tab_run, text="合并选项:").grid(row=row, column=0, sticky="w")
        frm_merge = ttk.Frame(tab_run)
        frm_merge.grid(row=row, column=1, columnspan=3, sticky="w")
        ...
        ttk.Label(frm_merge, text="模式:").pack(side="left", padx=(10, 2))
        ...
        # 引擎 & 进程策略
        ttk.Label(tab_run, text="转换引擎:").grid(row=row, column=0, sticky="w")
        frm_engine = ttk.Frame(tab_run)
        frm_engine.grid(row=row, column=1, columnspan=3, sticky="w")
        ...
        # 沙箱 & 临时目录
        ttk.Label(tab_run, text="沙箱 / 临时目录:").grid(row=row, column=0, sticky="nw")
        frm_sandbox = ttk.Frame(tab_run)
        frm_sandbox.grid(row=row, column=1, columnspan=3, sticky="ew")
    ...
    def start_task(self):
        ...
        source = self.var_source_folder.get().strip()
