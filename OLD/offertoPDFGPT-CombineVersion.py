# # -*- coding: utf-8 -*-
# """
# 项目名称: Office 文档批量转换为 PDF 工具 (内容策略版 + 报价智能识别 + 文件名优先 + 排除文件夹)
# 当前版本: v5.11.0
# 最后更新: 2025-04-01

# === 版本变更记录 ===
# v5.11.0:
#   - [新增] 排除文件夹功能：在 config.json 中配置 "excluded_folders" 列表。
#          支持排除特定名称的文件夹（如 "temp"）或特定路径。
#          程序扫描时会自动跳过这些文件夹及其子目录。

# v5.10.1:
#   - [优化] 修复 config.json 中文显示。
#   - [逻辑] 文件名优先匹配报价。
#   - [增强] Excel 深度扫描。
# """ 本本版本没有优化弹窗。5.11.1会优化
...
class OfficeConverter:
    ...
    def select_run_mode(self):
        print("=" * 60)
        print("请选择运行模式 (步骤 1/2):")
        print("  [1] 仅转换 (不做合并)")
        print("  [2] 仅合并 (根据前缀分类合并目标目录中的 PDF)")
        print("  [3] 先转换再合并 (推荐)")
        print("-" * 60)
        choice = input("请输入序号 (1/2/3，默认为 3): ").strip()
        if choice == '1':
    ...
    def select_content_strategy(self):
        print("-" * 60)
        print("请选择内容处理策略 (步骤 2/2):")
        print("  [1] 标准分类: 处理所有文件，不扫描内容，仅按后缀分类。")
        print("  [2] 智能标记: 【优先匹配文件名】或扫描内容，发现“报价”则标为 Price。")
        print("  [3] 报价猎手: 【仅处理】文件名包含关键词或内容包含“报价”的文件，跳过其他。")
        print("-" * 60)
        print(f"当前关键字: {self.price_keywords}")
        
        choice = input("请输入序号 (1/2/3，默认为 1): ").strip()
        if choice == '2':
    ...
    def setup_logging(self):
        ...
        with open(self.log_path, 'a', encoding='utf-8') as f:
            ...
            f.write(f"内容策略: {self.content_strategy}\n")
    ...
    def scan_excel_content_in_thread(self, workbook):
        """
        在 COM 线程中扫描 Excel 内容
        【升级】：遍历所有 Worksheets，而不仅仅是 ActiveSheet
        """
        try:
            # 遍历所有 Sheet
                ...
                try:
                    if self.engine_type == ENGINE_WPS:
                        ...
                        try:
                            ...
                            if os.path.exists(sandbox_target_pdf): os.remove(sandbox_target_pdf)
                            self._safe_exec(doc.SaveAs, sandbox_target_pdf, FileFormat=xlPDF_SaveAs)
                    else:
                        doc = self._safe_exec(app.Workbooks.Open, file_source, UpdateLinks=0, ReadOnly=True, IgnoreReadOnlyRecommended=True, CorruptLoad=xlRepairFile)
    ...
    def process_single_file(self, file_path, target_path_initial, ext, progress_str, is_retry=False):
        ...
        # 结果上下文：is_price 标记是否是报价单，skip_scan 标记是否跳过内容扫描（因为文件名已命中）
        result_context = {
            ...
            if is_pdf:
                ...
                if convert_thread.is_alive():
                    ...
                    if not self.reuse_process:
                        if is_word: self._kill_current_app('word')
                        elif is_excel: self._kill_current_app('excel')
                        elif is_ppt: self._kill_current_app('ppt')
                    raise Exception("超时")
    ...
    def run(self):
        if self.run_mode in (MODE_CONVERT_ONLY, MODE_CONVERT_THEN_MERGE):
            ...
            excl_names = {x.lower() for x in excl_config if not os.path.isabs(x) and os.sep not in x and '/' not in x}
            excl_paths = {os.path.abspath(x).lower() for x in excl_config if os.path.isabs(x) or os.sep in x or '/' in x}
            ...
            if not os.path.exists(self.config['source_folder']):
                ...
                for root, dirs, filenames in os.walk(self.config['source_folder']):
                    # 【核心修改】原地修改 dirs 以剪除排除的文件夹
                    # 必须修改 dirs 列表，os.walk 才会跳过这些目录
                    # 逻辑：保留那些 名字不在排除名中 且 绝对路径不在排除路径中 的目录
                    dirs[:] = [
                        d for d in dirs 
    ...
    if not os.path.exists(args.config):
        try:
            default_config = {
                ...
                "price_keywords": ["报价", "价格表", "Quotation"],
                # 新增排除文件夹示例
