...
class RunModeStateMixin:
    ...
    def _on_run_mode_change(self):
        ...
        self._on_toggle_fast_md_engine()
    ...
    def _apply_disabled_reason_tooltips(self):
        ...
        reason_fast_md = self.tr("tip_disabled_by_fast_md")
        ...
        is_merge_related = mode in (MODE_CONVERT_THEN_MERGE, MODE_MERGE_ONLY)
        fast_md_on = bool(
            getattr(self, "var_enable_fast_md_engine", None)
            and self.var_enable_fast_md_engine.get()
            and is_convert
        )
        ...
        for attr in (
            ...
            "chk_output_enable_pdf",
            "chk_output_enable_merged",
            "chk_output_enable_independent",
            "chk_enable_parallel_conversion",
        ):
            ...
            try:
                if _try_state(w) == "disabled":
                    if fast_md_on and attr in (
                        "chk_output_enable_pdf",
                        "chk_output_enable_merged",
                        "chk_output_enable_independent",
                        "chk_enable_parallel_conversion",
                    ):
                        setattr(w, "_tooltip_disabled_reason", reason_fast_md)
                    else:
    ...
    def _on_toggle_fast_md_engine(self):
        if not hasattr(self, "var_enable_fast_md_engine"):
            return
        mode = self.var_run_mode.get() if hasattr(self, "var_run_mode") else ""
        is_convert = mode in (MODE_CONVERT_ONLY, MODE_CONVERT_THEN_MERGE)
        enabled = bool(self.var_enable_fast_md_engine.get()) and is_convert

        if enabled:
            try:
                self.var_output_enable_md.set(1)
                self.var_output_enable_pdf.set(0)
                self.var_output_enable_merged.set(0)
                self.var_output_enable_independent.set(1)
                if hasattr(self, "var_enable_parallel_conversion"):
                    self.var_enable_parallel_conversion.set(0)
            except Exception:
                pass
        ...
        if enabled:
            for attr in (
                ...
                "chk_output_enable_independent",
                "chk_enable_merge",
                "rb_merge_submode_merge",
                "rb_merge_submode_pdf_to_md",
                "chk_enable_parallel_conversion",
            ):
                w = getattr(self, attr, None)
                if w is None:
            ...
            if hasattr(self, "_frm_parallel_sub"):
                try:
                    self._set_widget_tree_state(self._frm_parallel_sub, "disabled")
                except Exception:
                    pass
        else:
            if hasattr(self, "_on_toggle_parallel_conversion"):
                try:
