...
class ConverterBatchParallelSplitTests(unittest.TestCase):
    def test_batch_parallel_core_behaviors(self):
        ...
        root = tempfile.mkdtemp(prefix="batch_parallel_")
        ...
        class Dummy:
            def __init__(self):
                self.config = {"parallel_workers": 1, "parallel_checkpoint_interval": 1}
        ...
        dummy = Dummy()
        results = run_batch_parallel(dummy, [src], is_retry=False, source_alias_map=None)
    ...
    def test_office_converter_run_batch_parallel_delegates_to_module(self):
        ...
        original = oc.run_batch_parallel_impl
        try:
            seen = {}
            ...
            oc.run_batch_parallel_impl = _fake
            dummy = OfficeConverter.__new__(OfficeConverter)

            out = dummy.run_batch_parallel(["a.docx"], is_retry=True, source_alias_map={"a": "b"})
