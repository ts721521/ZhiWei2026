...
知识投喂工具：将 Office 文档批量转换为 PDF，支持合并/归集、任务中心、增量处理与 NotebookLM 溯源。

当前代码版本：`v5.20.0`（来自 `office_converter.py` 的 `__version__`）

## 核心能力

- 批量转换：Word / Excel / PowerPoint 转 PDF（WPS 或 Microsoft Office 引擎）。
- PDF 合并：按分类或全量合并，支持索引页、映射文件、书签短 ID。
- 任务中心（v5.20.0 唯一运行入口）：任务列表 + 向导式新建/编辑、批量运行、定时运行、"定时一览"窗口。
- 每任务独立配置：保存时生成 `config_profiles/task_<id>.json`，不同任务互不干扰。
- 内容去重：复制模式按 SHA256 自动去重并生成索引；转换模式可选按类型 MD5 全局去重（向导里可切）。
